<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register') }}">
        @csrf <!-- Token de verificação CSRF (Cross-Site Request Forgery) -->

        <!-- Nome -->
        <div>
            <x-input-label for="name" :value="__('Name')" /> <!-- Rótulo para o campo de nome -->
        ...
        <!-- Endereço de Email -->
        <div class="mt-4">
            <x-input-label for="email" :value="__('Email')" /> <!-- Rótulo para o campo de email -->
        ...
        <!-- Senha -->
        <div class="mt-4">
            <x-input-label for="password" :value="__('Password')" /> <!-- Rótulo para o campo de senha -->
        ...
        <!-- Confirmar Senha -->
        <div class="mt-4">
            <x-input-label for="password_confirmation" :value="__('Confirm Password')" /> <!-- Rótulo para o campo de confirmação de senha -->
        ...
        <div class="flex items-center justify-end mt-4">
            <a class="underline text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" href="{{ route('login') }}">
                {{ __('Already registered?') }} <!-- Link para a página de login -->
            </a>

            <!-- Botão de Registro -->
            <x-primary-button class="ms-4">
                {{ __('Register') }} <!-- Botão para registrar -->
            </x-primary-button>
        </div>
    </form>
</x-guest-layout>
