...
<style>
  ...
  *{box-sizing:border-box} html,body{height:100%}
  ...
  .card{
    width:min(94vw, 420px); padding:24px 22px 20px; border-radius:22px;
  ...
  .avatar{width:72px;height:72px;border-radius:50%;
    background:linear-gradient(180deg,#ffffff33,#ffffff11); border:1px solid var(--card-border);
...
<body>
  <main class="card">
    ...
    <p class="sub">Informe seu e-mail. Enviaremos um link para redefinir sua senha.</p>
    ...
    <form method="POST" action="{{ route('password.email') }}">
      ...
      <div class="field">
        ...
        <input id="email" name="email" type="email" required autofocus placeholder="seu@email.com" value="{{ old('email') }}">
      </div>
